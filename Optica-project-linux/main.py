...
def get_gpu_info():
    try:
        gpu_info = subprocess.check_output(["lspci", "-vnn", "-d", "10de:"], encoding="utf-8")
...
class GUI(customtkinter.CTk):
    def __init__(self):
        ...
        # INPUT FRAME LENGTH 
        self.entry_width = customtkinter.CTkEntry(self.Frame2, 
                                                        placeholder_text="mm")
        ...
        # INPUT FRAME HEIGHT
        self.entry_height = customtkinter.CTkEntry(self.Frame2, 
                                                    placeholder_text="mm")
    ...
    def add_faces(self):
        try:
            subprocess.run(['xdg-open', "{}//{}".format(PATH, L.Universal["Faces Folder"])])
    ...
    def open_results(self):
        try:
            subprocess.run(['xdg-open', "{}/{}".format(PATH, L.Universal["Ready Images Folder"])])
        except Exception as error:
    ...
    @error_handler
    def tutorial(self):
        subprocess.run(['vlc', "{}/tutorial.mp4".format(L.Universal["Necessary Files Folder"])])
    ...
    def put_glasses(self, ImageInput=None): #function to put glasses on the face
        ...
        self.width_Oculos = int(self.width * self.pixel_mm_ratio) # sets the width of the Oculos image to be the same as the distance between 2 points of the face
        mask_Oculos = mask_Oculos.resize((self.width_Oculos, int(self.height * self.pixel_mm_ratio))) # resizes the glasses to the correct width.
